...
test("It displays the table sorted by created at on descending order", function () {
    ...
    livewire(ParticipantList::class, ['event' => Event::latest()->first()])->assertSeeInOrder([$participants[0]->first_name, $participants[1]->first_name]);
    get('/' . Event::latest()->first()->id)->assertSeeInOrder([$participants[0]->first_name, $participants[1]->first_name]);
});
